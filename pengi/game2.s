	...
	XDEF	OutOfMemory,back,MenuInt,InitDisk
	...
	XDEF	PosX,PosY,MSpr1,MSpr2,MSpr3,MSpr4,GSpr1,GSpr2,GSpr3,GSpr4,_GetJoy,_SetSprPos
	XDEF	CheckBonus,BPage,MemoryHeader,GetName2
	XDEF	StartLevel,ActLevel,ShowPerfect,CheckSound,SoundFlag,FxFlag
	XDEF	CheckPause,CheckBobs,FadeOutSong
	XDEF	SetFlag,EndBob3,EndBob4
	XDEF	HiScoreCopper3,HiColors3

	XDEF	SetSound,MusicVBL,LockVoices,SongPtr,MasterVol
	XDEF	Decruncher,Button

	XDEF	InitKey,RestoreKey,GetKey,KeyBoard
	XDEF	Read,Write,FxBase

BobLenght:	EQU	60800
planel:		EQU	19200
pagel:		EQU	5*planel

top:		EQU	$80000-1000		; $7fc18
page1:		EQU	top-pagel		; $68518
page2:		EQU	page1-pagel		; $50e18

back:		EQU	page2-38464		; $477d8
panel:		EQU	back-6464		; $45e98
FontBase:	EQU	panel-19064		; $41420
BobBase:	EQU	FontBase-BobLenght	; $326a0
SongBase:	EQU	BobBase-104200		; $18f98
FxBase:		EQU	SongBase-21690		; $13ad4
...
1$:		move.l	d0,(a0)+
		...
		move	#$7fff,custom+intena
		move	#$7fff,custom+intreq
		move.l	#BlackCopper,custom+cop1lc

		lea	MFMBuffer,a0		; MFMBuffer
		moveq	#1,d1			; 1 Drive
		move.l	#PRODUCT,d0		; Produkt-Code
		bsr	InitDisk
...
c3:		LoadFile	FxBase,FN_FX,43*512
c4:		LoadCruFile	SongBase,FN_BIGFILE,333*512
c11:		LoadFile	hilist,FN_HISCORES,512
...
c10:		LoadFile	page1,FN_MENU,40448
...
14$:		cmp	#2,YPos
		...
		LoadCruFile	page2,FN_HELP,66*512
		lea	HiColors3+2,a0
...
Begin2:		moveq	#1,d0
		...
		move.l	#BobBuffer,MemoryHeader
		move.l	#BobBuffer+$3000,MemoryHeader+4
		bsr	InitAlloc
		bsr	InitVars
		bsr	ClearPages
		bsr	SetColors
...
HandleLevel:	;sub.l	#126,LevelPtr
		bra	NextLevel

NoNextLevel:	tst	CheatFlag
		bne	NoLivesSub
...
SprList:	dc.l	MSpr1,MSpr2,MSpr3,MSpr4
SprList2:	dc.l	GSpr1,GSpr2,GSpr3,GSpr4  ;MusicSpr1,MusicSpr2,0,0
...
EndBob3:	move.l	Bob3,a4
		...
		clr	80(A4)
		add.l	#1000,Score
		PlayFx	FxBase+Broesel,816,1428,45,64
		rts
...
565$:		move.l	Bob1,a0
		...
		PlayFx	FxBase+WhizMove,1253,6514,228,64
		bra	NoMove2

500$:		cmp.b	#23,d7			; Death
		bne	502$
		move.b	#'E',EndFlag
		bra	510$
...
611$:		move	FirstDefs+12,d2
		...
		PlayFx	FxBase+Explosion,1253,2484,70,64
		bra	NoMove2

504$:		cmp.b	#57,d7			; Bonus
		bne	505$

		add.l	#1000,Score
		PlayFx	FxBase+Pling,400,419,5,40
...
532$:
		...
		PlayFx	FxBase+Broesel,1016,1428,45,64
		bra	NoMove
...
102$:
		...
		PlayFx	FxBase+Broesel,1016,1428,50,64
...
FadeOutCopper:	cmove	$2081+16,diwstrt
		cmove	$06c1-16,diwstop
		cmove	$30+16,ddfstrt
		cmove	$d0-16,ddfstop
		cmove	$5200,bplcon0
...
HiScoreCopper3:	cmove	$2081,diwstrt
		cmove	$ffc1,diwstop
		cmove	$38,ddfstrt
		cmove	$d0,ddfstop
		cmove	$5200,bplcon0
...
copperl:	cmove	$2081+16,diwstrt
		cmove	$fdc1-16,diwstop
		cmove	$30+8,ddfstrt
		cmove	$d0-8,ddfstop
		cmove	$5200,bplcon0
...
ScrollText:	DC.B	'    LEONARDO    ',-1,200
		...
		DC.B	'PROGRAMMING :     RENE STRAUB   ',-1,175
		DC.B	'          '
		...
		DC.B	'    '
		DC.B	'USING SOUND FX          '
		DC.B	'ADDITIONAL PROGRAMMING :   CHRIS HALLER   ',-1,148
		DC.B	'          '
		DC.B	'   CHRISTIAN WEBER  ',-1,150
		DC.B	'          '
		DC.B	'LEVEL DESIGN :    RETO STRAUB   ',-1,100
		DC.B	'          '
		DC.B	'PRODUCER   :     HEINZ LUEM    ',-1,99
		DC.B	'          '
		DC.B	0
